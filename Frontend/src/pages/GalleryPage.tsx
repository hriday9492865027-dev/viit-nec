...
function ImageCard({ img, index, onClick }: { img: GalleryImage; index: number; onClick: () => void }) {
  const { isVisible, ref } = useScrollAnimation(0.05);
  return (
    <div
      ref={ref as React.RefObject<HTMLDivElement>}
      onClick={onClick}
      className={`gallery-card group relative h-72 rounded-2xl overflow-hidden cursor-pointer shadow-sm border border-slate-200 transition-all duration-600 ${
    ...
    >
      ...
      <div className="overlay absolute inset-0 bg-gradient-to-t from-blue-950/80 via-blue-950/20 to-transparent opacity-70 group-hover:opacity-95 transition-opacity duration-500" />
      <div className="absolute inset-0 p-5 flex flex-col justify-between z-10 text-white">
        ...
        <div>
          <h3 className="text-base font-bold text-white group-hover:text-blue-200 transition-colors line-clamp-1">{img.title}</h3>
...
export default function GalleryPage() {
  ...
  useEffect(() => { fetchImages(); }, [showOnlyFeatured]);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-6 border-b border-slate-200 pb-8">
            <div className="space-y-2">
              <Link to="/" className="inline-flex items-center gap-2 text-xs font-bold text-blue-700 hover:text-blue-900 transition-colors mb-2">
            ...
            <div className="flex items-center gap-3">
              ...
              <Link to="/admin" className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-blue-700 hover:bg-blue-800 text-white text-xs font-bold uppercase tracking-wider shadow-md transition-all hover:-translate-y-0.5">
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
              {images.map((img, i) => (
                <ImageCard key={img.id} img={img} index={i} onClick={() => setSelectedImage(img)} />
              ))}
            </div>
          )}

          <LightboxModal image={selectedImage} images={images} onClose={() => setSelectedImage(null)} onSelectImage={setSelectedImage} />
        </div>
      </main>
      <Footer />
    </div>
