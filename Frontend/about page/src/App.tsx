...
function AnimatedCount({ target, suffix = '' }: { target: number; suffix?: string }) {
  ...
  return (
    <span ref={ref} style={{ fontFamily: 'var(--font-head)', fontSize: 'clamp(2rem,4vw,2.8rem)', fontWeight: 700, color: C.gold, display: 'block', lineHeight: 1 }}>
      {count}{suffix}
    </span>
  )
}
...
function HeroSection() {
  ...
  return (
    <section style={{ position: 'relative', height: '100vh', minHeight: 700, overflow: 'hidden', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <div style={{ position: 'absolute', inset: 0, background: `linear-gradient(to bottom, rgba(22,58,54,0.72) 0%, rgba(22,58,54,0.55) 40%, rgba(22,58,54,0.82) 100%)`, zIndex: 1 }} />
      ...
      {/* Content */}
      <div style={{ position: 'relative', zIndex: 5, textAlign: 'center', padding: '0 2rem', maxWidth: 800, width: '100%' }}>
        {/* Pill */}
        <div style={{
          ...
          <span style={{ fontFamily: 'var(--font-head)', fontSize: '0.72rem', letterSpacing: '0.16em', color: C.gold, textTransform: 'uppercase', fontWeight: 500 }}>About VIIT E-Cell</span>
        </div>
        ...
        <p style={{
          ...
          maxWidth: 660, margin: '0 auto 3.5rem',
        ...
        <div style={{
          ...
          {STATS.map((s, i) => (
            <div key={s.label} className="stat-card glass" style={{
      ...
      <div style={{
        ...
        <div style={{ width: 1, height: 40, background: `linear-gradient(to bottom, ${C.gold}, transparent)`, animation: 'scroll-bob 2s ease-in-out infinite' }} />
      </div>
...
function AboutSection() {
  return (
    <section style={{ background: C.green, padding: '7rem 2rem', position: 'relative', overflow: 'hidden' }}>
      <Blobs variant="dark" />
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', position: 'relative', zIndex: 1 }}>
        ...
        <Reveal>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '4rem' }}>
            ...
            <span style={{ fontFamily: 'var(--font-body)', fontSize: '0.68rem', letterSpacing: '0.28em', textTransform: 'uppercase', color: C.goldDim }}>Our Foundation</span>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '2rem', alignItems: 'stretch' }}>
          ...
          <Reveal delay={0.1} style={{ gridColumn: 'span 1' }}>
            <div className="gold-border-shimmer" style={{
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.6rem' }}>
                {CHIPS.map(c => (
                  <div key={c.label} className="chip glass" style={{
...
function VisionSection() {
  ...
  return (
    <section style={{ background: C.teal, padding: '7rem 2rem', position: 'relative', overflow: 'hidden' }}>
      <Blobs variant="light" />
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', pointerEvents: 'none', zIndex: 0 }}>
        <svg viewBox="0 0 800 400" style={{ width: '90%', maxWidth: 900, opacity: 0.04 }}>
          ...
          {Array.from({ length: 24 }, (_, i) => {
            const x = 40 + (i % 8) * 100; const y = 60 + Math.floor(i / 8) * 140
            return <circle key={i} cx={x} cy={y} r={4} fill={C.gold} />
          })}
          {Array.from({ length: 20 }, (_, i) => {
            const x1 = 40 + (i % 7) * 100; const y1 = 60 + Math.floor(i / 7) * 140
            const x2 = x1 + 100 + (Math.random() * 100 - 50); const y2 = y1 + (Math.random() * 80 - 40)
        ...
        <Reveal delay={0.1}>
          ...
          >
            ...
            <h2 style={{ fontFamily: 'var(--font-head)', fontSize: 'clamp(2rem,4.5vw,3.2rem)', fontWeight: 700, color: C.white, lineHeight: 1.15, marginBottom: '1.5rem', letterSpacing: '-0.025em' }}>
              To be the{' '}
              <span style={{ color: C.gold, textShadow: `0 0 30px rgba(230,196,138,0.35)` }}>premier launchpad</span>
            ...
            <p style={{ fontFamily: 'var(--font-body)', fontSize: 'clamp(0.92rem,1.8vw,1.05rem)', color: 'rgba(215,215,215,0.75)', lineHeight: 1.8, fontWeight: 300, maxWidth: 620, margin: '0 auto' }}>
              We envision a thriving innovation ecosystem within VIIT where every student with a bold idea finds the mentorship, resources, and community to turn their vision into a scalable, impactful venture — redefining what's possible from a campus.
            </p>
          </div>
        </Reveal>
      </div>
...
const MISSIONS = [
  ...
  },
  {
    ...
    desc: 'Equipping students with the practical toolkit of entrepreneurship — product thinking, financial literacy, negotiation, pitching, and leadership — through workshops, bootcamps, and live challenges.',
  },
  {
    icon: '🤝',
    step: '03',
    title: 'Strategic Collaboration',
    desc: 'Forging partnerships with industry leaders, angel investors, VCs, and government bodies to create real pathways from classroom ideas to funded, market-ready companies.',
  },
  {
    icon: '⚖️',
    step: '04',
    title: 'Intellectual Property',
    desc: 'Guiding student innovators through the IP landscape — patents, trademarks, and copyrights — ensuring their breakthroughs are protected and positioned to create lasting value.',
  },
]
...
function MissionSection() {
  return (
    <section style={{ background: C.green, padding: '7rem 2rem 8rem', position: 'relative', overflow: 'hidden' }}>
      <Blobs variant="dark" />
      ...
      <div style={{ maxWidth: 720, margin: '0 auto', textAlign: 'center', position: 'relative', zIndex: 1 }}>
        ...
        <Reveal delay={0.2}>
          <div style={{ marginTop: '5rem', paddingTop: '2rem', borderTop: `1px solid rgba(230,196,138,0.08)`, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.5rem' }}>
            <span style={{ fontFamily: 'var(--font-body)', fontSize: '0.62rem', letterSpacing: '0.2em', color: 'rgba(215,215,215,0.28)', textTransform: 'uppercase' }}>© VIIT E-Cell 2025</span>
